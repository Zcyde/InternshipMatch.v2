...
import { AuthService } from './auth'; // Correctly import the service
...
export class AppComponent {
  // Use 'public' so the HTML template can access 'auth' signals
    ...
    try {
      ...
      if (response.ok) {
        if (this.isSignupMode) {
          ...
          // Success for Login - updating the global signals
          if (data.role === "admin") {
            this.auth.isAdmin.set(true);
            this.auth.isUser.set(false);
          } else {
            this.auth.isAdmin.set(false);
            this.auth.isUser.set(true);
          }
          this.loginError = false;
        }
      } else {
      ...
      this.loginError = true;
      this.loginErrorMessage = "Network or server error occurred.";
      alert(this.loginErrorMessage);
    }

    this.passInput = ""; // Clear password field
  }
